...
class TestInstrumentList(APITestCase):
    ...
    def test_create_instrument(self):
        ...
        compare(response.status_code, status.HTTP_201_CREATED)
        compare(Instrument.objects.count(), 1)
        compare(Instrument.objects.get(id=1).symbol, 'TSLA')
...
class TestHoldingList(APITestCase):
    def setUp(self):
        ...
        self.trade_dict = collections.OrderedDict(
            {'holding': self.holding, 'date': '2020-09-08', 'category': 'BUY', 'broker': 'Freetrade', 'price_currency': 'USD', 'quantity': 1.0, 'unit_price': 1.0,
             'payment_currency': 'USD', 'fee': 1.0, 'tax': 1.5, 'fx_rate': 1.2})
    ...
    def test_create_holding(self):
        ...
        compare(response.status_code, status.HTTP_201_CREATED)
        compare(Holding.objects.count(), 2)
        compare(Holding.objects.get(id=2).username, 'b')
    ...
    def test_list_valid_username_returns_holdings(self):
        ...
        compare(response.data, [expect_holding_dict])
...
class TestTradeList(APITestCase):
    def setUp(self):
        ...
        self.trade_dict = collections.OrderedDict(
            {'holding': self.holding, 'date': '2020-09-08', 'broker': 'Freetrade', 'category': 'BUY', 'price_currency': 'USD', 'quantity': 1, 'unit_price': 1,
             'payment_currency': 'GBP', 'fee': 1, 'tax': 1.5, 'fx_rate': 1.2})
        self.trade = Trade.objects.create(**self.trade_dict)

    def test_create_trade(self):
        request_data = {'holding': 1, 'date': '2020-09-10', 'broker': 'Freetrade', 'price_currency': 'GBP',
                        'quantity': 2, 'unit_price': 1, 'payment_currency': 'USD', 'fee': 1, 'tax': 1.5, 'fx_rate': 1.2}
        response = self.client.post(self.url, request_data, format='json')

        compare(response.status_code, status.HTTP_201_CREATED)
        compare(Trade.objects.count(), 2)
        compare(Trade.objects.get(id=2).date,
    ...
    def test_list_valid_holding_returns_trades(self):
        ...
        expect_trade_dict.move_to_end('id', last=False)
        expect_trade_dict['holding'] = self.holding.id
        compare(response.data, [expect_trade_dict])
...
class TestTradeDetail(APITestCase):
    def setUp(self):
        ...
        self.trade_1 = Trade.objects.create(
            ...
            fee=4, tax=1.5, fx_rate=1.2)
        self.trade_2 = Trade.objects.create(
            holding_id=1, date='2020-09-11', quantity=0.4, unit_price=201,
            fee=4, tax=1.5, fx_rate=1.1)

    def test_delete_trade(self):
        response = self.client.delete(self.url)
        compare(response.status_code, status.HTTP_204_NO_CONTENT)
        compare(list(Trade.objects.all()), [self.trade_2])
